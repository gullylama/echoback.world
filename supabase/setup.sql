-- EchoBack — one-shot Supabase setup
-- Paste this whole file into the Supabase SQL editor and run it once.
-- (It is migrations 0001 + 0002 plus the private audio storage bucket.)
...
-- ---------------------------------------------------------------------------
-- Private storage bucket for raw audio (uploads are served via the server
-- with signed access only — no public reads).
-- ---------------------------------------------------------------------------
...
-- A subscriber sends a request on a match; the recipient reads it, hears the
-- track and accepts or declines FOR FREE. Accepting opens the thread.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Match threshold + no self-matching (migration 0004)
-- ---------------------------------------------------------------------------
create or replace function refresh_matches_for_track(p_track_id uuid)
returns void
language plpgsql
